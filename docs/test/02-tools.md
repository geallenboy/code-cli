...
```bash
node -e "import('./dist/tools/editor.js').then(m => console.log(m.writeFile('/tmp/gearcode-test.txt', 'hello world')))"
cat /tmp/gearcode-test.txt
```

**预期**：`File written: /tmp/gearcode-test.txt (1 lines)`，文件内容为 `hello world`
...
```bash
rm -f /tmp/gearcode-test.txt /tmp/test-edit.ts /tmp/test-dup.ts /tmp/test-agent-edit.ts
```
