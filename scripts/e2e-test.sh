...
echo -e "${GREEN}Mini Claude Code — 端到端自动化测试${NC}"
...
if echo "$OUTPUT" | grep -q "mini-claude-code"; then
  pass "read_file — 读取 package.json"
else
  fail "read_file" "未找到 mini-claude-code"
fi
...
if echo "$OUTPUT" | grep -q "read_file\|mini-claude"; then
...
SESSION_COUNT=$(ls ~/.mini-claude/sessions/ 2>/dev/null | wc -l | tr -d ' ')
